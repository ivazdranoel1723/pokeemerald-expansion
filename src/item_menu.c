...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        ...
        else if (gSpecialVar_ItemId && (offset = RegisteredItemIndex(gSpecialVar_ItemId)) >= 0)
        {
    ...
    default:
        ...
        else
        {
            ...
            case POCKET_KEY_ITEMS:
                ...
                else if (GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_InfiniteCandy){
...
static u32 CountRegisteredItems(void) {
    ...
    if (count == 0 && gSaveBlock1Ptr->registeredItem) {
...
s32 RegisteredItemIndex(u16 item) {
    ...
    for (i = 0; i < ARRAY_COUNT(gSaveBlock1Ptr->registeredItems); i++)
        if (gSaveBlock1Ptr->registeredItems[i] && (!item || gSaveBlock1Ptr->registeredItems[i] == item))
            return i;
    if (item && item == gSaveBlock1Ptr->registeredItem) {
...
static void Task_KeyItemWheel(u8 taskId)
{
    ...
    case 2:
        ...
        ItemUseFunc func = GetItemFieldFunc(gSaveBlock1Ptr->registeredItem); i = CreateTask(func, 8);
