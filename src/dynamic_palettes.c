...
void DynPal_LoadIntroToneIndices(void)
{
    u16 maleFrontId = PlayerGenderToFrontTrainerPicId(0);
    u16 femaleFrontId = PlayerGenderToFrontTrainerPicId(1);

    DynPal_ReloadPlayerPaletteForMenu(maleFrontId, 0, 0, 0);
    DynPal_ReloadPlayerPaletteForMenu(femaleFrontId, 0, 2, 0);
}
...
static void DynPal_ReloadPlayerPaletteForMenu(u16 paletteTag, u8 partATone, u8 partBTone, u8 partCTone)
{
    ...
    {
        // For non-overworld palettes, resolve the OBJ palette index from the sprite palette tag.
        // This works for both Emerald/Expansion and FRLG where palettes are registered by tag.
